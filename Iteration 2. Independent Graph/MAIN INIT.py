...
soft_colors = ['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd', '#8c564b', '#e377c2', '#7f7f7f', '#bcbd22',
               '#17becf']
hard_colors = ['#000000', '#772211', '#000066', '#555555', '#0022cc', '#ffcc00']
colmap_clash = ColorMap([0.0, 0.2, 0.6, 1.0],
...
time_splits = [('years', 2 * 365 * 24 * 60 * 60, 'YS', 4), ('months', 3 * 30 * 24 * 60 * 60, 'MS', 10),
    ...
    def tickValues(self, minVal, maxVal, size):
        ...
        for mode, dtt, freq, ticklen in time_splits:
            if dts > dtt:
                ...
                steps = len(rng) if len(rng) & 1 == 0 else len(
                    rng) + 1  # reduce jitter between e.g. 5<-->10 ticks for resolution close to limit
...
class PandasDataSource:
    ...
    def __init__(self, df):
        ...
        self.scale_cols = [i for i in range(self.col_data_offset, len(self.df.columns)) if
                           self.df.iloc[:, i].dtype != object]
    ...
    def calc_significant_decimals(self, full):
        ...
        def calc_sd(ser):
            ...
            for _ in range(2):  # check if we have a remainder that is a better epsilon
                ...
                if smallest_diff * 0.05 < smallest_diff_r < smallest_diff * 0.7 and remainder_ok(smallest_diff,
                                                                                                 smallest_diff_r):
    ...
    def is_smooth_time(self):
        if self._smooth_time is None:
            ...
            self._smooth_time = self.timebased() and (
                        np.abs(np.diff(self.x.values[1:100])[1:] // (self.period_ns // 1000) - 1000) < 10).all()
    ...
    def addcols(self, datasrc):
        ...
        datasrc.is_sparse = datasrc.is_sparse or datasrc.df[
            datasrc.df.columns[datasrc.col_data_offset]].isnull().sum().max() > ldf2
...
class FinCrossHair:
    ...
    def update(self, point=None):
        ...
        sd, se = (self.ax.significant_decimals, self.ax.significant_eps) if clamp_grid else (significant_decimals,
                                                                                             significant_eps)
        timebased = False
...
class FinViewBox(pg.ViewBox):
    ...
    def mouseLeftDrag(self, ev, axis):
        ...
        select_band_drag = not self.drawing and (
                    self.vband is not None or ev.modifiers() == QtCore.Qt.KeyboardModifier.ShiftModifier)
        draw_drag = self.vband is None and (
                    self.drawing or ev.modifiers() == QtCore.Qt.KeyboardModifier.ControlModifier)
        ...
        if draw_drag:
            ...
            if not self.drawing:
                ...
                p0 = _clamp_point(self.parent(), p0)
                self.draw_line = _create_poly_line(self, [p0, p1], closed=False, pen=pg.mkPen(draw_line_color),
                                                   movable=False)
...
class FinPlotItem(pg.GraphicsObject):
    ...
    def _generate_picture(self, boundingRect):
        ...
        self.cachedRect = QtCore.QRectF(boundingRect.left() - (cache_candle_factor - 1) * 0.5 * w, 0,
                                        cache_candle_factor * w, 0)
...
class HeatmapItem(FinPlotItem):
    def __init__(self, ax, datasrc, rect_size=0.9, filter_limit=0, colmap=colmap_clash, whiteout=0.0,
        ...
        for t, row in enumerate(values):
            for ci, price in enumerate(prices):
                ...
                if v >= lim:
                    ...
                    p.fillRect(QtCore.QRectF(t - rect_size2, self.ax.vb.yscale.invxform(price + h0), self.rect_size,
                                             self.ax.vb.yscale.invxform(h1)), color)


class HorizontalTimeVolumeItem(CandlestickItem):
    def __init__(self, ax, datasrc, candle_width=0.8, draw_va=0.0, draw_vaw=1.0, draw_body=0.4, draw_poc=0.0,
                 colorfunc=None):
        ...
        super().__init__(ax, datasrc, draw_shadow=False, candle_width=candle_width, draw_body=draw_body,
                         colorfunc=colorfunc)
        self.lod = False
...
def create_plot_widget(master, rows=1, init_zoom_periods=1e10, yscale='linear'):
    ...
    if isinstance(master, pg.GraphicsLayoutWidget):
        proxy = pg.SignalProxy(master.scene().sigMouseMoved, rateLimit=144,
                               slot=partial(_mouse_moved, master, axs[0].vb))
        ...
        for ax in axs:
            proxy = pg.SignalProxy(ax.ax_widget.scene().sigMouseMoved, rateLimit=144,
                                   slot=partial(_mouse_moved, master, ax.vb))
...
def candlestick_ochl(datasrc, draw_body=True, draw_shadow=True, candle_width=0.6, ax=None, colorfunc=price_colorfilter):
    ...
    item = CandlestickItem(ax=ax, datasrc=datasrc, draw_body=draw_body, draw_shadow=draw_shadow,
                           candle_width=candle_width, colorfunc=colorfunc, resamp='hilo')
...
def volume_ocv(datasrc, candle_width=0.8, ax=None, colorfunc=volume_colorfilter):
    ...
    _set_datasrc(ax, datasrc)
    item = CandlestickItem(ax=ax, datasrc=datasrc, draw_body=True, draw_shadow=False, candle_width=candle_width,
                           colorfunc=colorfunc, resamp='sum')
    _update_significants(ax, datasrc, force=True)
    item.colors['bull_body'] = item.colors['bull_frame']
...
def plot(x, y=None, color=None, width=1, ax=None, style=None, legend=None, zoomscale=True, **kwargs):
    ...
    if style is None or any(ch in style for ch in '-_.'):
        ...
        item = ax.plot(x, y, pen=_makepen(color=used_color, style=style, width=width), name=legend,
                       connect=connect_dots)
        item.setZValue(5)
    else:
        ...
        item = ax.plot(x, ser.values, pen=None, symbol=symbol, symbolPen=None, symbolSize=7 * width,
                       symbolBrush=pg.mkBrush(used_color), name=legend)
        if width < 1:
...
def add_horizontal_band(y0, y1, color=band_color, ax=None):
    ...
    lr = pg.LinearRegionItem([ix(y0), ix(y1)], orientation=pg.LinearRegionItem.Horizontal, brush=pg.mkBrush(color),
                             movable=False)
...
def add_vertical_band(x0, x1, color=band_color, ax=None):
    ...
    color = _get_color(ax, None, color)
    lr = pg.LinearRegionItem([x_pts[0], x_pts[1]], orientation=pg.LinearRegionItem.Vertical, brush=pg.mkBrush(color),
                             movable=False)
    lr.lines[0].setPen(pg.mkPen(None))
    lr.lines[1].setPen(pg.mkPen(None))
    lr.setZValue(-50)
...
def add_rect(p0, p1, color=band_color, interactive=False, ax=None):
    ...
    rect = FinRect(ax=ax, brush=pg.mkBrush(color), pos=pos, size=size, movable=interactive, resizable=interactive,
                   rotatable=False)
    rect.setZValue(-40)
    if interactive:
...
def set_mouse_callback(callback, ax=None, when='click'):
    ...
    if when == 'hover':
        master.proxy_hover = pg.SignalProxy(master.scene().sigMouseMoved, rateLimit=15,
                                            slot=partial(_mcallback_pos, ax, callback))
    elif when in ('dclick', 'double-click'):
        master.proxy_dclick = pg.SignalProxy(master.scene().sigMouseClicked,
                                             slot=partial(_mcallback_click, ax, callback, 'dclick'))
    elif when in ('click', 'lclick'):
        master.proxy_click = pg.SignalProxy(master.scene().sigMouseClicked,
                                            slot=partial(_mcallback_click, ax, callback, 'lclick'))
    elif when in ('mclick',):
        master.proxy_click = pg.SignalProxy(master.scene().sigMouseClicked,
                                            slot=partial(_mcallback_click, ax, callback, 'mclick'))
    elif when in ('rclick',):
        master.proxy_click = pg.SignalProxy(master.scene().sigMouseClicked,
                                            slot=partial(_mcallback_click, ax, callback, 'rclick'))
    elif when in ('any',):
        master.proxy_click = pg.SignalProxy(master.scene().sigMouseClicked,
                                            slot=partial(_mcallback_click, ax, callback, 'any'))
    else:
...
def _create_datasrc(ax, *args, ncols=-1, allow_scaling=True):
    ...
    if len(datasrc.df.columns) in (1, ncols - 1):
        ...
        if len(datasrc.df.columns) in (1, ncols - 1):
            if ncols > 1:
                print(f"WARNING: this type of plot wants %i columns/args, but you've only supplied %i" % (ncols,
                                                                                                          len(datasrc.df.columns)))
...
pg.widgets.GraphicsView.GraphicsView.wheelEvent = partialmethod(_wheel_event_wrapper,
