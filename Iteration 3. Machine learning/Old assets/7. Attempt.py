...
if __name__ == "__main__":
    ...
    while True:
        ...
        df = yf.download(target, period="2y", interval="1d", auto_adjust=False, progress=False)
        ...
        # 5. CREATE THE CONTINUOUS FORECAST LINE
        # Anchor points: Today, +1d, +7d (1w), +30d (1m)
        forecast_dates = [last_date] + [preds[h]['target_date'] for h in horizons.keys()]
        forecast_prices = [curr_p] + [preds[h]['price'] for h in horizons.keys()]

        # Create a Series indexed by these specific dates
        f_series = pd.Series(forecast_prices, index=forecast_dates)

        # Reindex to include ALL dates in the extended range and interpolate
        # This creates the "straight line" through the gaps
        f_series = f_series.reindex(df_extended.index).interpolate(method='linear')

        # 6. PLOT
        ax = fplt.create_plot(f"AI Forecast: {target} (21-Day Outlook)")

        # Plot extended candles (the NaN part is invisible but creates the space)
        fplt.candlestick_ochl(df_extended[['Open', 'Close', 'High', 'Low']], ax=ax)

        # Plot the continuous forecast path
        fplt.plot(f_series, ax=ax, color='#00efff', style='--', width=2, legend='AI Path')
        ...
        for h, label in horizons.items():
            ...
            print(f" > {label} Target: ${t_price:.2f} (Confidence: {preds[h]['prob']:.0%})")
